switch (state)
{
    case roll_state.ROLL:
    {
        ...
        repeat (_steps)
        {
	        ...
	        if (check_collision(_dir, -4*pixel_size, [objCrate, objCrateBig, objCrateStrawberry, prtRollSpawner])) // we check slightly above so the melon will roll over 1-tile holes
	        {
        ...
        else
        {
	        ...
	        while (
	            place_meeting(x, y, [objCollisionSemiSolid]) || check_collision(0, 0, [prtCrate, prtRollSpawner])
	        )
	        {
	            y -= 1;
	        }
    ...
    case roll_state.SPLAT:
    {
        audio_stop_sound(snd_inst_thunder);

        repeat (choose(1, 2, 2, 2, 3))
            instance_create_layer(x, y, "instances", splat_object, { splat_id: "down" });

        repeat (choose(1, 2, 2, 2, 3))
            instance_create_layer(x, y, "instances", splat_object, { splat_id: "left" });

        repeat (choose(1, 2, 2, 2, 3))
            instance_create_layer(x, y, "instances", splat_object, { splat_id: "right" });
